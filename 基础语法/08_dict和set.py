...
# 第一种方式：
print('Bob' in d)
# 第二种方式：通过dict提供的get(key)方法，如果key不存在返回None；如果存在则返回对应的value
print(d.get('Bob'))
print(d.get('Tom'))
# 还可以指定当key不存在时返回特定的值，示例：
print(d.get('Tom',-1)) # 当dict中不存在key='Tom'时，返回-1

# pop(key):删除特定的key,返回值为key所对应的value
d.pop('Jerry')
print(d)

# 注意：dict中数据的存取不一致

# dict利用key通过hash算法来计算value的存储位置，所以不可变的对象(例如，整数、字符串等)才可以作为key，而list是可变的就不能作为key
l = [1,'b',3.122]
# d[l] = 'Hello'
# print(d[l])
